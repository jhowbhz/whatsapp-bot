    ...
    try {
        ...
        if (event.action == 'add' && isWelkom) {
            ...
            if( dddPermitidos.includes(parseInt(ddd)) ){
                ...
                await client.sendText(event.chat, `Oi *novato(a)*, 👋 seja bem vindo(a) leia as regras e se apresente ao grupo com _nome, foto, idade e cidade_ ou será banido.`, pChat.id)

            }else{

                await client.sendText(event.chat, `Seu DDD *${ddd}* não é permitido nesse grupo você será removido! HAHAHA`)
                setTimeout( async()=>{
